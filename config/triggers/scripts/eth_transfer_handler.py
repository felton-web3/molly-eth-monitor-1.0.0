...
def setup_logging():
    ...
    log_file = log_dir / f"eth-{today}.log"
...
def save_transfer_data(conn, transfer_data, verbose=False):
    ...
    try:
        ...
        if verbose:
            logger.info(f"成功插入ETH转账记录: {transfer_data['tx_hash']}")
        else:
            logger.info(f"成功插入ETH转账记录: {transfer_data['tx_hash']}")
...
def process_eth_transfer(input_data, verbose=False):
    """处理ETH转账数据"""
    logger = logging.getLogger(__name__)
    try:
        # 解析输入数据
        ...
        if verbose:
            logger.info(f"收到ETH转账数据: {json.dumps(data, indent=2)}")
...
def main():
    ...
    try:
        ...
        # 处理ETH转账
        if process_eth_transfer(input_data):
            logger.info("ETH转账处理完成")
            sys.exit(0)
        else:
            logger.error("ETH转账处理失败")
            sys.exit(1)
