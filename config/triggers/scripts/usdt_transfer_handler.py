...
def setup_logging():
    ...
    log_file = log_dir / f"usdt-{today}.log"
...
def save_transfer_data(conn, transfer_data, verbose=False):
    ...
    try:
        ...
        if verbose:
            logger.info(f"成功插入USDT转账记录: {transfer_data['tx_hash']}")
        else:
            logger.info(f"成功插入USDT转账记录: {transfer_data['tx_hash']}")
...
def process_usdt_transfer(input_data, verbose=False):
    """处理USDT转账数据"""
    logger = logging.getLogger(__name__)
    try:
        # 解析输入数据
        ...
        if verbose:
            logger.info(f"收到USDT转账数据: {json.dumps(data, indent=2)}")
            logger.info("数据结构分析:")
...
def main():
    ...
    try:
        ...
        # 处理USDT转账
        if process_usdt_transfer(input_data):
            logger.info("USDT转账处理完成")
            sys.exit(0)
        else:
            logger.error("USDT转账处理失败")
            sys.exit(1)
